...
#%% TIME DOMAIN
wave_22 = few_base(M, mu, p0, e0, theta, phi,T=T,dt=dt,eps=1e-2)#mode_selection=[(l,m,n)],include_minus_m=True)  , batch_size=int(1e2),mode_selection=[(l,m,n)])#,include_minus_m=True) #
freq_fft = np.fft.fftfreq(len(wave_22),dt)
fft_wave = np.fft.fftshift(np.fft.fft( np.real(wave_22) )*dt)
...
fd_h_correct = real_fd #-np.roll( np.flip(np.real(fd_h)) + 1j* np.flip(np.imag(fd_h)), 1)#np.sin(dt*len(wave_22)*freq_fft/4/np.pi)/np.sin(dt*freq_fft/4/np.pi)#*np.exp(-1j* (len(wave_22)-1)/2 )
...
# FD model
plt.plot( np.real(fd_h_correct),'--',alpha=0.9,label='FD domain waveform' )
