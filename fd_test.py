...
fft_wave = np.fft.fftshift(np.fft.fft(wave_22))*dt
...
fd_h_correct = -( np.flip(np.real(fd_h)) + 1j* np.flip(np.imag(fd_h)) )
...
# FD model
plt.plot(freq_fft, -np.flip(np.real(fd_h)),'--',alpha=0.9,label='FD domain waveform' )
plt.legend()
plt.show()
...
# FD model
plt.plot(f, -np.flip(np.imag(fd_h)),'--',alpha=0.9,label='FD domain waveform' )
plt.legend()
plt.show()
# %%
