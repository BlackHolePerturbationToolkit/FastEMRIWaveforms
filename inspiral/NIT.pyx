...
cdef extern from "../inspiral/include/FluxInspiral.hh":
    void NITWrapper(np.float64_t *t, np.float64_t *p, np.float64_t *e, np.float64_t *Phi_phi, np.float64_t *Phi_r, np.float64_t M, np.float64_t mu, np.float64_t p0, np.float64_t e0, int*, np.float64_t err)


def NIT(M, mu, p0, e0, err=1e-10):
    ...
    NITWrapper(&t[0], &p[0], &e[0], &Phi_phi[0], &Phi_r[0], M, mu, p0, e0, &length, err)
