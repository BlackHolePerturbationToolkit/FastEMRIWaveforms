...
KERRECCEQ_AMP_TEST_POINTS = [
    {
        ...
        "Almkn": 0.002753336140076555 + 0.004143260615912325j,
    },
    {
        "lmkn": (2, -2, 0, 5),
        "ape": (0.77593827, 5.44830674, 0.39950367),
        "Almkn": -0.0001975310651632842 + 4.806588136412258e-05j,
    },
    {
        "lmkn": (2, 2, 0, 0),
        "ape": (0.99090693, 1.81708312, 0.25313127),
        "Almkn": -0.09712303568244392 + 0.0004771647068539275j,
    },
    {
        "lmkn": (4, -3, 0, 1),
        "ape": (0.87600927, 43.70360964, 0.478125),
        "Almkn": -9.89242565478408e-07 + 1.750469365836169e-05j,
    },
    {
        "lmkn": (3, 3, 0, 0),
        "ape": (0.36158837, 72.94420732, 0.39375),
        "Almkn": 5.869887391976143e-05 + 0.00136010620358342j,
    },
]
...
class AmplitudesTest(FewBackendTest):
    ...
    def test_kerrecceq(self):
        ...
        for test_point in KERRECCEQ_AMP_TEST_POINTS:
            ...
            mode_interp = amp_module(a, p, e, xI, specific_modes=[test_point["lmkn"]])[
                test_point["lmkn"]
            ].item()
        np.testing.assert_allclose(mode_interp, test_point["Almkn"], rtol=0, atol=1e-9)

        # test the mode symmetry
        specific_modes = [(3, 2, 0, 1), (3, -2, 0, -1)]
        specific_amplitudes = amp_module(a, p, e, xI, specific_modes=specific_modes)
        if self.backend.uses_gpu:
            np.testing.assert_allclose(
                specific_amplitudes[(3, 2, 0, 1)].get(),
                -specific_amplitudes[(3, -2, 0, -1)].conj().get(),
                rtol=1e-10,
            )
        else:
            np.testing.assert_allclose(
                specific_amplitudes[(3, 2, 0, 1)],
