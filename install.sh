...
if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    if [[ "$machine" == "arm64" ]]; then
        ...
        conda create -n "$env_name" -c conda-forge -y wget hdf5 numpy Cython scipy tqdm jupyter ipython h5py requests matplotlib python=3.12 openblas lapack liblapacke rich numba
    else
        echo "This is not an M1 Mac."
        conda create -n "$env_name" -c conda-forge -y clangxx_osx-64 clang_osx-64 wget lapack=3.6.1 hdf5 numpy Cython scipy tqdm jupyter ipython h5py requests matplotlib rich numba python=3.12
    fi
elif [[ "$OSTYPE" == "linux-gnu"* ]]; then
    echo "This system is Unix/Linux."
    conda create -n "$env_name" -c conda-forge -y gcc_linux-64 gxx_linux-64 wget lapack=3.6.1 hdf5 rich numba numpy Cython scipy tqdm jupyter ipython h5py requests matplotlib python=3.12
else
