...
The compiled module will have to be declared in `src/few/cutils/CMakeLists.txt` by adding a new section similar to those that start with the headers of the form
...
See [here](deploy.md#understanding-the-cmake-compilation-mechanism) for details about the compilations detailed in those sections.
