...
class InterpolatedModeSumGeneric(SummationBase, GenericWaveform, ParallelModuleBase):
    """Create waveform by interpolating sparse trajectory.
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.use_gpu:
            self.get_waveform = get_waveform_generic_wrap

        else:
            self.get_waveform = get_waveform_generic_wrap_cpu

    def attributes_InterpolatedModeSumGeneric(self):
        """
        attributes:
            get_waveform (func): CPU or GPU function for waveform creation.
