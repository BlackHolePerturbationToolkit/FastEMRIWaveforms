...
class GenericModeSelector(ParallelModuleBase):
    """Filter teukolsky amplitudes based on power contribution.
    ...
    def __init__(self, l_arr, m_arr, k_arr, n_arr, sensitivity_fn=None, **kwargs):
        ParallelModuleBase.__init__(self, **kwargs)
    ...
    def __call__(self, teuk_modes, modeinds, fund_freq_args=None, eps=1e-5):
        """Call to sort and filer teukolsky modes.
        ...
        # get the power contribution of each mode, with R and L modes interleaved
        power = (xp.abs(teuk_modes.reshape(teuk_modes.shape[0], -1)) ** 2)
        ...
        # finds indices of each mode to be kept
        temp = inds_sort[inds_keep] // 2

        # if R or L contributes, keep both due to structure of CUDA kernel
        keep_modes = xp.unique(temp)

        # setup up teuk mode returns
        out1 = (teuk_modes[:, keep_modes],)
