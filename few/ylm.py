...
class GetYlms:
    ...
    def __call__(self, l_in, m_in, theta, phi):
        ...
        ylms_dict = {
            (2, -2): (np.sqrt(5 / np.pi) * costheta ** 4)
            / (2.0 * np.exp(2 * 1j * phi)),
            (2, -1): -(
                (np.sqrt(5 / np.pi) * costheta ** 3 * sintheta) / np.exp(1j * phi)
            ),
            ...
            * sintheta ** 6,
            ...
            (5, -4): (
                np.sqrt(33 / np.pi)
                * (
                    3 * costheta ** 8 * sintheta ** 2
                    - 7 * costheta ** 6 * sintheta ** 4
                )
            )
            / np.exp(4 * 1j * phi),
            (5, -3): -(
            ...
            * sintheta ** 10,
            ...
            (9, -8): (
                np.sqrt(8398 / np.pi)
                * (
                    7 * costheta ** 12 * sintheta ** 6
                    - 11 * costheta ** 10 * sintheta ** 8
                )
            )
            / np.exp(8 * 1j * phi),
            (9, -7): (
