...
def get_ylms(l, m, theta, phi, buffer):
    ...
    ylms_dict = {
        (2, -2): (np.sqrt(5 / np.pi) * costheta ** 4) / (2.0 * np.exp(2 * 1j * phi)),
        (2, -1): -((np.sqrt(5 / np.pi) * costheta ** 3 * sintheta) / np.exp(1j * phi)),
        (2, 0): np.sqrt(15 / (2.0 * np.pi)) * costheta ** 2 * sintheta ** 2,
        (2, 1): -(np.exp(1j * phi) * np.sqrt(5 / np.pi) * costheta * sintheta ** 3),
        (2, 2): (np.exp(2 * 1j * phi) * np.sqrt(5 / np.pi) * sintheta ** 4) / 2.0,
        (3, -3): (np.sqrt(21 / (2.0 * np.pi)) * costheta ** 5 * sintheta)
        / np.exp(3 * 1j * phi),
        (3, -2): (
            np.sqrt(7 / np.pi) * (costheta ** 6 - 5 * costheta ** 4 * sintheta ** 2)
        )
        / (2.0 * np.exp(2 * 1j * phi)),
        (3, -1): -(
            (
                np.sqrt(7 / (10.0 * np.pi))
                * (5 * costheta ** 5 * sintheta - 10 * costheta ** 3 * sintheta ** 3)
            )
            / np.exp(1j * phi)
        ),
        (3, 0): (
            np.sqrt(21 / (10.0 * np.pi))
            * (10 * costheta ** 4 * sintheta ** 2 - 10 * costheta ** 2 * sintheta ** 4)
        )
        / 2.0,
        (3, 1): -(
            np.exp(1j * phi)
            * np.sqrt(7 / (10.0 * np.pi))
            * (10 * costheta ** 3 * sintheta ** 3 - 5 * costheta * sintheta ** 5)
        ),
        (3, 2): (
            np.exp(2 * 1j * phi)
            * np.sqrt(7 / np.pi)
            * (5 * costheta ** 2 * sintheta ** 4 - sintheta ** 6)
        )
        / 2.0,
        (3, 3): -(
            np.exp(3 * 1j * phi)
            * np.sqrt(21 / (2.0 * np.pi))
            * costheta
            * sintheta ** 5
        ),
        (4, -4): (3 * np.sqrt(7 / np.pi) * costheta ** 6 * sintheta ** 2)
        / np.exp(4 * 1j * phi),
        (4, -3): (
            -3
            * np.sqrt(7 / (2.0 * np.pi))
        ...
        / (2.0 * np.exp(2 * 1j * phi) * np.sqrt(np.pi)),
        (4, -1): (
            -3
            * (
        ...
        / (2.0 * np.exp(1j * phi) * np.sqrt(2 * np.pi)),
        (4, 0): (
            3
            * (
        ...
        * costheta ** 2
        * sintheta ** 6,
        (5, -5): (np.sqrt(330 / np.pi) * costheta ** 7 * sintheta ** 3)
        / np.exp(5 * 1j * phi),
        (5, -4): (
            np.sqrt(33 / np.pi)
        ...
        * (
        ...
        (5, 4): np.exp(4 * 1j * phi)
        * np.sqrt(33 / np.pi)
        * (7 * costheta ** 4 * sintheta ** 6 - 3 * costheta ** 2 * sintheta ** 8),
        (5, 5): -(
            np.exp(5 * 1j * phi) * np.sqrt(330 / np.pi) * costheta ** 3 * sintheta ** 7
        ),
        (6, -6): (3 * np.sqrt(715 / np.pi) * costheta ** 8 * sintheta ** 4)
        / (2.0 * np.exp(6 * 1j * phi)),
        (6, -5): -(
            np.sqrt(2145 / np.pi)
            * (-4 * costheta ** 9 * sintheta ** 3 + 8 * costheta ** 7 * sintheta ** 5)
        )
        / (4.0 * np.exp(5 * 1j * phi)),
        (6, -4): (
            np.sqrt(195 / (2.0 * np.pi))
            * (
        ...
        (6, 5): -(
            np.exp(5 * 1j * phi)
            * np.sqrt(2145 / np.pi)
            * (8 * costheta ** 5 * sintheta ** 7 - 4 * costheta ** 3 * sintheta ** 9)
        )
        / 4.0,
        (6, 6): (
            3
        ...
        / 2.0,
        (7, -7): (np.sqrt(15015 / (2.0 * np.pi)) * costheta ** 9 * sintheta ** 5)
        / np.exp(7 * 1j * phi),
        (7, -6): (
            np.sqrt(2145 / np.pi)
            * (5 * costheta ** 10 * sintheta ** 4 - 9 * costheta ** 8 * sintheta ** 6)
        )
        / (2.0 * np.exp(6 * 1j * phi)),
        (7, -5): -(
            (
                np.sqrt(165 / (2.0 * np.pi))
                * (
        ...
        (7, 6): (
            np.exp(6 * 1j * phi)
            * np.sqrt(2145 / np.pi)
            * (9 * costheta ** 6 * sintheta ** 8 - 5 * costheta ** 4 * sintheta ** 10)
        )
        / 2.0,
        (7, 7): -(
            np.exp(7 * 1j * phi)
            * np.sqrt(15015 / (2.0 * np.pi))
            * costheta ** 5
            * sintheta ** 9
        ),
        (8, -8): (np.sqrt(34034 / np.pi) * costheta ** 10 * sintheta ** 6)
        / np.exp(8 * 1j * phi),
        (8, -7): -(
            np.sqrt(17017 / (2.0 * np.pi))
            * (-6 * costheta ** 11 * sintheta ** 5 + 10 * costheta ** 9 * sintheta ** 7)
        )
        / (2.0 * np.exp(7 * 1j * phi)),
        (8, -6): (
            np.sqrt(17017 / (15.0 * np.pi))
            * (
        ...
        * (
        ...
        (8, 7): -(
            np.exp(7 * 1j * phi)
            * np.sqrt(17017 / (2.0 * np.pi))
            * (10 * costheta ** 7 * sintheta ** 9 - 6 * costheta ** 5 * sintheta ** 11)
        )
        / 2.0,
        (8, 8): np.exp(8 * 1j * phi)
        * np.sqrt(34034 / np.pi)
        * costheta ** 6
        * sintheta ** 10,
        (9, -9): (6 * np.sqrt(4199 / np.pi) * costheta ** 11 * sintheta ** 7)
        / np.exp(9 * 1j * phi),
        (9, -8): (
            np.sqrt(8398 / np.pi)
        ...
        * (
        ...
        (9, 8): np.exp(8 * 1j * phi)
        * np.sqrt(8398 / np.pi)
        * (11 * costheta ** 8 * sintheta ** 10 - 7 * costheta ** 6 * sintheta ** 12),
        (9, 9): -6
        * np.exp(9 * 1j * phi)
        * np.sqrt(4199 / np.pi)
        * costheta ** 7
        * sintheta ** 11,
        (10, -10): (
            3 * np.sqrt(146965 / (2.0 * np.pi)) * costheta ** 12 * sintheta ** 8
        )
        / np.exp(10 * 1j * phi),
        (10, -9): (
            -3
            * np.sqrt(29393 / (2.0 * np.pi))
            * (
        ...
        * (
        ...
        (10, 9): (
            ...
            * np.exp(9 * 1j * phi)
            * np.sqrt(29393 / (2.0 * np.pi))
            * (12 * costheta ** 9 * sintheta ** 11 - 8 * costheta ** 7 * sintheta ** 13)
        )
        / 2.0,
        (10, 10): 3
        * np.exp(10 * 1j * phi)
        * np.sqrt(146965 / (2.0 * np.pi))
        * costheta ** 8
        * sintheta ** 12,
    }
