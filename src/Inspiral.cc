...
TestCase::TestCase(ODECarrier* testcarrier_, std::string func_name, bool enforce_schwarz_sep_, int num_add_args_, bool convert_Y_, std::string few_dir)
{
    params_holder = new ParamsHolder;
    params_holder->func_name = func_name;
    params_holder->enforce_schwarz_sep = enforce_schwarz_sep_;
...
void TestCase::run_test(double *t, double *p, double *e, double *x, double *Phi_phi, double *Phi_theta, double *Phi_r, double M, double mu, double a, double p0, double e0, double x0, double Phi_phi0, double Phi_theta0, double Phi_r0, int *length, double tmax, double dt, double err, int DENSE_STEPPING, bool use_rk4, int init_len, double* additional_args)
{
    double t0 = 0.0;
    std::memcpy(params_holder->additional_args, additional_args, params_holder->num_add_args * sizeof(double));
    InspiralHolder Inspiral_vals = run_Inspiral2(t0, M, mu, a, p0, e0, x0, Phi_phi0, Phi_theta0, Phi_r0, err, tmax, dt, DENSE_STEPPING, use_rk4);
...
InspiralHolder TestCase::run_Inspiral2(double t0, double M, double mu, double a, double p0, double e0, double x0, double Phi_phi0, double Phi_theta0, double Phi_r0, double err, double tmax, double dt, int DENSE_STEPPING, bool use_rk4)
{
    // years to seconds
    tmax = tmax*YRSID_SI;
