...
def get_backend(backend_name: str) -> Backend:
    ...
    if backend_name == "cuda":
        return get_first_backend(["cuda13x", "cuda12x", "cuda11x"])
    if backend_name == "gpu":
...
def has_backend(backend_name: str) -> bool:
    ...
    If the backend name is "cuda", return true if any of "cuda13x", "cuda12x" or "cuda11x" is available.
    If the backend name is "gpu", return true if any GPU backend is available.
    """
    if backend_name == "cuda":
        return has_backend("cuda13x") or has_backend("cuda12x") or has_backend("cuda11x")
    if backend_name == "gpu":
