...
def get_cuda_version() -> tuple[int, int]:
    ...
    try:
        ...
        raise exceptions.CudaException("CUDA driver not loaded: could not detect a CUDA version.") from e
    except pynvml.NVMLError as e:
        raise exceptions.CudaException("CUDA exception: could not detect a CUDA version.") from e
...
def check_cupy_works():
    ...
    except exceptions.CudaException as e:
        raise exceptions.CuPyException("CuPy does not work due to CUDA error.") from e
    ...
    try:
        ...
        raise exceptions.MissingDependency(f"CuPy is not installed. Run `pip install cupy-cuda{cuda_version[0]}x` to install it.") from e

    # Try basic CuPy function to check real-time compilation is working
    try:
        _ = cupy.arange(1)
    except cupy.cuda.compiler.CompileException as e:
        raise exceptions.MissingDependency(f"CuPy fails to run due to missing dependencies. Run `pip install nvidia-cuda-runtime-cu{cuda_version[0]}` to install them.") from e
    except (cupy_backends.cuda.api.runtime.CUDARuntimeError, RuntimeError) as e:
        raise exceptions.CuPyException("CuPy could not execute properly.") from e
...
def try_import_cuda11x() -> ModuleType:
    ...
    try:
        ...
        raise exceptions.BackendUnavailable("CUDA 11.x backend is not available: check CUDA drivers are properly installed on your system.") from e

    if cuda_version[0] != 11:
        raise exceptions.BackendUnavailable(f"CUDA 11.x backend is not available: CUDA driver version should be 11.x but is {cuda_version[0]}.{cuda_version[1]}")

    try:
        from .. import cuda11x
    except ImportError as e:
        raise exceptions.BackendUnavailable("CUDA 11.x backend is not importable.") from e
    return cuda11x
...
def try_import_cuda12x() -> ModuleType:
    ...
    try:
        ...
        raise exceptions.BackendUnavailable("CUDA 12.x backend is not available: check CUDA drivers are properly installed on your system.") from e

    if cuda_version[0] != 12:
        raise exceptions.BackendUnavailable(f"CUDA 12.x backend is not available: CUDA driver version should be 12.x but is {cuda_version[0]}.{cuda_version[1]}")

    try:
        from .. import cuda12x
    except ImportError as e:
        raise exceptions.BackendUnavailable("CUDA 12.x backend is not importable.") from e
    return cuda12x
...
def try_import_nvidia_so_libs(libs: Sequence[tuple[str, str, str]]) -> None:
    ...
    if failed_idx:
        ...
        raise exceptions.MissingDependency(f"Could not load the following NVidia libraries: {failed_libs}. "
                                f"If you installed FEW using pip, you may install them by running "
                                f"`pip install {packages}`. ") from exceptions.ExceptionGroup("Following exceptions were raised when trying to load these libraries", exceptions)
...
def try_preload_cuda12x_dynlibs():
    ...
    if sys.platform == "linux":
        ...
        try_import_nvidia_so_libs(cuda12x_solibs)

def try_preload_cuda11x_dynlibs():
    ...
    if sys.platform == "linux":
        ...
        try_import_nvidia_so_libs(cuda11x_solibs)


def force_import_cuda11x_backend() -> ModuleType:
    """Force import the cuda11x backend."""
    # If this method is called, the CUDA driver is installed with version 12.x.

    try:
        try_import_cuda11x()
    except exceptions.BackendNotInstalled as e:
        raise exceptions.BackendUnavailable("Backend CUDA 11.x is not installed. "
                                 "Run `pip install fastemriwaveforms-cuda11x` to install it.") from e
    except exceptions.BackendUnavailable:
        pass

    # Check that nvidia dynamic libraries are available
    try:
        try_preload_cuda11x_dynlibs()
    except exceptions.MissingDependency as e:
        raise exceptions.BackendUnavailable("Some CUDA libraries required by FEW are not available. Please follow previous instructions.") from e

    # Then check that CuPy works
    try:
        check_cupy_works()
    except exceptions.CuPyException as e:
        raise exceptions.BackendUnavailable("Your system has CUDA 11.x installed but CuPy does not work. Follow previous instructions.") from e
...
def force_import_cuda12x_backend() -> ModuleType:
    ...
    try:
        ...
        raise exceptions.BackendUnavailable("Backend CUDA 12.x is not installed. "
    ...
    try:
        ...
        raise exceptions.BackendUnavailable("Your system has CUDA 12.x installed but CuPy does not work. Follow previous instructions.") from e
...
def import_lazy_backend() -> ModuleType:
    ...
    try:
        cuda_version = get_cuda_version()
    except exceptions.CudaException:
    ...
    try:
        ...
        check_cupy_works()
        return try_import_cuda12x()
    except exceptions.FewException:
        pass
