...
def get_waveform_fd_wrap(*args, **kwargs):
    ...
    (waveform,
           ...
           turnover_freqs, max_points, df, f_data, zero_index, shift_freq, slope0_all, initial_freqs) = targs
...
def interp_time_for_fd(*args, **kwargs):

    targs, kwargs = wrapper(*args, **kwargs)
    
    (output, t_arr, tstar, ind_tstar, interp_array, ninterps, length, run) = targs
...
def find_segments_fd(*args, **kwargs):

    targs, kwargs = wrapper(*args, **kwargs)
    
    (segment_out, start_inds_seg, end_inds_seg, mode_start_inds, num_segments, num_modes, max_length) = targs
