...
for aa in np.arange(0,10):
    el = f'./files/a0.{aa}0_xI1.000.flux'
    imp = np.loadtxt(el)
...
# get correct ordering of the fluxes
reshapedF = np.asarray([[[ get_pdot(el1,el2,el3) for el3 in x3] for el2 in x2] for el1 in x1])

# flux interpolation
InterpFlux = TP_Interpolant_ND(X, F=reshapedF)

cdef public void call_quack(double* c, double* X, double y, int dimX, int dimc, int Dim):
    bw_array_ptrs = <gsl_bspline_workspace **>malloc(Dim * sizeof(gsl_bspline_workspace*));
    cdef int ret = TP_Interpolation_ND(c, dimc, <double*> X, dimX, bw_array_ptrs, &y)